...
namespace IsoGen
{
    ...
    /// In addition, the combined list of vertices is sorted in ascending order by Z, then by Y, then by X.
    /// </summary>
    public record Rectangle : Quadrilateral
    {
            ...
            : base(triangles.orig, triangles.refl)
        {
            ...
            // Sort the combined vertices: first by Z, then by Y, then by X (all in ascending order).
            Vertices.Sort((v1, v2) =>
            {
                int cmp = v1.Z.CompareTo(v2.Z);
                if (cmp == 0)
                {
                    cmp = v1.Y.CompareTo(v2.Y);
                    if (cmp == 0)
                        cmp = v1.X.CompareTo(v2.X);
                }
                return cmp;
            });
